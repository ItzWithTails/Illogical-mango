#!/usr/bin/env python3
"""
niri-config.py — Niri compositor configuration helper for Illogical-mango settings UI.
...
Commands:
  ...
  detect-customizations Report Niri files that differ from shipped Illogical-mango defaults
        ...
        add_file(
            {
                ...
                "reason": "This managed Niri file differs from the shipped Illogical-mango default.",
            ...
            add_file(
                {
                    ...
                    if relative_path == "config.d/15-outputs.kdl"
                    else "Additional user config file not shipped by Illogical-mango defaults.",
                    "preview": preview,
...
def _sync_cursor_env(theme=None, size=None):
    ...
    The install may place XCURSOR_THEME in ilmango.conf or the user might have
    ...
    target_file = None
    for candidate in ["ilmango.conf", "cursor.conf"]:
...
# niri-config.py is called with a full path from the ilmango launcher.
...
def _kb_parse_ilmango_action(action: str):
    """Detect ilmango IPC calls and return (target, function) or None."""
    m = re.search(
        r'spawn\s+"(?:[^"]*/)?ilmango"\s+"ipc"\s+"call"\s+"([\w-]+)"\s+"([\w-]+)"', action
    )
    if m:
        return m.group(1), m.group(2)
    if re.search(r'spawn\s+"(?:[^"]*/)?ilmango"\s+"settings"(?:\s|;|$)', action):
        return "settings", "open"
    if re.search(r'spawn\s+"(?:[^"]*/)?ilmango"\s+"terminal"(?:\s|;|$)', action):
        return "launcher", "terminal"
    if re.search(r'spawn\s+"(?:[^"]*/)?ilmango"\s+"close-window"(?:\s|;|$)', action):
        return "launcher", "close-window"
    if re.search(r'spawn\s+"(?:[^"]*/)?ilmango"\s+"browser"(?:\s|;|$)', action):
        return "browser", "open"
    m = re.search(r'spawn\s+"(?:[^"]*/)?ilmango"\s+"([\w-]+)"\s+"([\w-]+)"', action)
    if m:
        return m.group(1), m.group(2)
    return None
...
def _kb_generate_comment(action: str) -> str:
    ...
    if action.startswith("spawn"):
        ilmango = _kb_parse_ilmango_action(action)
        if ilmango:
            return _KB_IPC_MAP.get(ilmango, f"{ilmango[0]} {ilmango[1]}")
