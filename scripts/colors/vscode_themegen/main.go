...
const (
	themeName        = "Illogical-mango Material"
	themeExtensionID = "ilmango-material-theme"
	themeFileName    = "ilmango-material-color-theme.json"
	prevThemeKey     = "ilmango.previousColorTheme"
)
...
var legacyExtensionDirs = []string{"ilmango.ilmango-theme-1.0.0"}
...
func ensureExtensionRegistered(forkKey, extDir string) error {
	...
	extID := "ilmango." + themeExtensionID
...
func main() {
	...
	stripMode := flag.Bool("strip", false, "Remove Illogical-mango theme extension and clean settings.json")
...
func generateThemeForFork(colorsPath, terminalJSONPath, scssPath, settingsPath, forkKey string) (bool, error) {
	...
	// Clean up old-style injection from previous Illogical-mango versions
...
// stripThemeForFork removes the Illogical-mango extension and cleans settings.json
func stripThemeForFork(settingsPath, forkKey string) bool {
	...
	// Clean up legacy injection keys from any previous Illogical-mango version
	for _, key := range []string{
		"workbench.colorCustomizations",
		"editor.tokenColorCustomizations",
		"editor.semanticTokenColorCustomizations",
