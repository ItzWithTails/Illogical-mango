...
VSCODE_THEMEGEN_BIN="$STATE_DIR/user/generated/bin/ilmango-vscode-themegen"
OPENCODE_THEMEGEN_BIN="$STATE_DIR/user/generated/bin/ilmango-opencode-themegen"
...
# Only deletes if the file was generated by Illogical-mango (checks for known markers).
strip_neovim_spec() {
  [[ -f "$NEOVIM_THEME_FILE" ]] || return 0

  if grep -Fq '"yukazakiri/ilmango.nvim"' "$NEOVIM_THEME_FILE" || grep -Fq '"bjarneo/aether.nvim"' "$NEOVIM_THEME_FILE" || grep -Fq 'name = "ilmango-neovim"' "$NEOVIM_THEME_FILE"; then
