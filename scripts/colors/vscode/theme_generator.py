...
def generate_vscode_colors(colors, scss_path):
    """Generate VSCode workbench.colorCustomizations from Illogical-mango theme data."""
...
def _write_extension_manifest(ext_dir: Path) -> None:
    ...
    manifest = {
        ...
        "description": "Dynamic Material You theme — auto-generated by Illogical-mango from wallpaper colors",
        "version": "1.0.0",
        "publisher": "ilmango",
...
LEGACY_EXTENSION_DIRS = ["ilmango.ilmango-theme-1.0.0"]


def _clean_legacy_extensions(fork_key: str) -> None:
    """Remove old extension directories from previous Illogical-mango versions."""
    import shutil
...
def _ensure_extension_registered(fork_key: str, ext_dir: Path) -> None:
    ...
    ext_id = f"ilmango.{THEME_EXTENSION_ID}"
    for e in entries:
...
def generate_vscode_theme(colors_json_path, scss_path, settings_path, fork_key=""):
    ...
    # Clean up legacy extensions from older Illogical-mango versions
    ...
    # Remove legacy injection keys from previous Illogical-mango versions
    for key in [
...
THEME_NAME = "Illogical-mango Material"
THEME_EXTENSION_ID = "ilmango-material-theme"
THEME_FILE_NAME = "ilmango-material-color-theme.json"
PREV_THEME_KEY = "ilmango.previousColorTheme"
...
def strip_vscode_theme(settings_path: str, fork_key: str = "") -> bool:
    """Remove Illogical-mango theme extension and settings.json entries."""
    import shutil
    ...
    if changed:
        try:
            with open(settings_path, "w") as f:
                json.dump(settings, f, indent=2, ensure_ascii=False)
            print(f"✓ Stripped Illogical-mango theme from {settings_path}")
        except OSError as e:
...
def strip_all_vscode_themes(forks: list = None):
    """Strip Illogical-mango theme from multiple VSCode forks."""
    if forks is None:
        forks = [
            key
...
def generate_all_vscode_themes(
    ...
    Args:
        colors_json_path: Path to Illogical-mango palette.json (colors.json fallback is still accepted)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--strip",
        action="store_true",
        help="Remove Illogical-mango color customizations from settings.json (used when disabling theming)",
    )
