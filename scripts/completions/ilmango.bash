# Bash completion for ilmango CLI
# Install: eval "$(ilmango completions bash)"
# Or: ilmango completions bash > /etc/bash_completion.d/ilmango

_ilmango_completions() {
    local cur prev
    COMPREPLY=()
    ...
    if [[ -L "$ilmango_bin" ]] && command -v readlink &>/dev/null; then
...
complete -F _ilmango_completions ilmango
