...
# accounting out of ilmango.service so desktop monitors report the shell itself,
# not a completed batch's reclaimable file cache. The scope forwards stdout,
# preserving QML's machine-progress parser.
if [[ -z "${ILMANGO_THUMBGEN_SCOPED:-}" ]] && command -v systemd-run >/dev/null 2>&1; then
    exec systemd-run --user --scope --quiet --collect \
        --property="Description=Illogical-mango wallpaper thumbnails" \
        --setenv=ILMANGO_THUMBGEN_SCOPED=1 \
