...
# Updates Mod+T and Mod+Return to use the Illogical-mango launcher so the Settings UI
...
migration_check() {
  ...
  # Already migrated?
  if grep -qE 'Mod\+(T|Return).*spawn.*"ilmango"\s+"terminal"' "$config"; then
    return 1
  fi
...
migration_preview() {
  ...
  echo ""
  echo -e "${STY_GREEN}+ Mod+T { spawn \"ilmango\" \"terminal\"; }${STY_RST}"
  echo -e "${STY_GREEN}+ Mod+Return { spawn \"ilmango\" \"terminal\"; }${STY_RST}"
}
