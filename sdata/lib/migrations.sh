# Migration system for Illogical-mango
...
if declare -F resolve_ilmango_config_dir >/dev/null; then
    _ilmango_config_dir="$(resolve_ilmango_config_dir)"
else
    _xdg_config_home="${XDG_CONFIG_HOME:-$HOME/.config}"
    _config_new="${_xdg_config_home}/ilmango"
    _config_prev="${_xdg_config_home}/inir"
    _config_legacy="${_xdg_config_home}/illogical-impulse"

    if [[ -L "$_config_legacy" && -d "$_config_new" ]]; then
        _ilmango_config_dir="$_config_new"
    elif [[ -d "$_config_legacy" && ! -L "$_config_legacy" ]]; then
        _ilmango_config_dir="$_config_legacy"
    elif [[ -d "$_config_prev" && ! -L "$_config_prev" ]]; then
        _ilmango_config_dir="$_config_prev"
...
MIGRATIONS_STATE_FILE="${_ilmango_config_dir}/migrations.json"
MIGRATIONS_BACKUP_DIR="${_ilmango_config_dir}/backups"
