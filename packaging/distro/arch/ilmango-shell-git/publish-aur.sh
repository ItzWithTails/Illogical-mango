...
if ! curl -sf "https://raw.githubusercontent.com/ItzWithTails/illogical-mango/dev/LICENSE" >/dev/null 2>&1; then
...
cp "${PKG_DIR}/ilmango-shell-git.install" "${WORK_DIR}/ilmango-shell-git.install"
...
ls -la "${WORK_DIR}"/{PKGBUILD,.SRCINFO,ilmango-shell-git.install}

# Step 5: Commit and push
echo "[5/5] Committing and pushing to AUR..."
cd "${WORK_DIR}"
git add PKGBUILD .SRCINFO ilmango-shell-git.install
git commit -m "Initial upload: ilmango-shell-git $(grep pkgver= PKGBUILD | head -1 | cut -d= -f2)"
