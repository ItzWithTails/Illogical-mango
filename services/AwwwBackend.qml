    ...
    readonly property string _ensureDaemonScript: `
                ...
                if [ -n "$awww_bin" ] && [ -n "$daemon_bin" ] && ! "$awww_bin" query >/dev/null 2>&1; then
                    if command -v systemd-run >/dev/null 2>&1; then
                        if ! systemd-run --user --quiet --collect --property=Description="Illogical-mango wallpaper daemon" --setenv="WAYLAND_DISPLAY=$WAYLAND_DISPLAY" --setenv="XDG_RUNTIME_DIR=$XDG_RUNTIME_DIR" -- "$daemon_bin" >/dev/null 2>&1; then
