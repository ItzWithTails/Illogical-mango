...
/*
 * Illogical-mango shell update checker service.
...
Singleton {
    ...
    readonly property string manifestPath: configDir + "/.ilmango-manifest"
    ...
    readonly property string unavailableMessage: managedExternally
        ? "This Illogical-mango installation is managed outside the runtime copy. Use your package manager or installation workflow to update it."
        : "Repository not found. The update system cannot locate the Illogical-mango git repository."
    ...
    onHasUpdateChanged: {
        ...
        Notifications.notify({
            summary: "Illogical-mango Update Available" + version,
    ...
    function performUpdate(): void {
        ...
        const termTail =
            "echo; " +
            "if [ $rc -eq 0 ]; then " +
                "echo 'All good — Illogical-mango updated successfully. The shell will restart on its own.'; " +
        ...
        if (useTerminal) {
            ...
            ShellExec.execDetachedArgs(
                ...
                "Update Illogical-mango", repoDir)
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // `setup update` triggers `systemctl --user restart ilmango.service` as its
    ...
    Process {
        ...
        command: [
            ...
            "for dir in ~/illogical-impulse ~/illogical-mango ~/Illogical-mango ~/ilmango ~/inir ~/iNiR " +
            "~/.local/src/illogical-impulse ~/.local/src/illogical-mango ~/.local/src/ilmango ~/.local/src/inir " +
            "~/Projects/illogical-impulse ~/Projects/illogical-mango ~/Projects/ilmango ~/Projects/inir " +
            "~/Downloads/illogical-impulse ~/Downloads/illogical-mango ~/Downloads/ilmango ~/Downloads/inir " +
            "~/src/illogical-impulse ~/src/illogical-mango ~/src/ilmango ~/src/inir; do " +
            "if [[ -d \"$dir/.git\" && -f \"$dir/setup\" && -f \"$dir/shell.qml\" ]]; then echo \"$dir\"; exit 0; fi; done; " +
            // Last resort: find in home (max depth 3, timeout 2s)
            "timeout 2 find \"$HOME\" -maxdepth 3 -name setup \\( -path '*/illogical-mango/setup' -o -path '*/ilmango/setup' -o -path '*/inir/setup' -o -path '*/illogical-impulse/setup' -o -path '*/ii/setup' \\) 2>/dev/null | while read -r f; do [[ -f \"$(dirname \"$f\")/shell.qml\" ]] && dirname \"$f\" && break; done; "
        ]
        stdout: StdioCollector {
            onStreamFinished: {
        ...
        onExited: (exitCode, exitStatus) => {
            if (exitCode !== 0) {
                ...
                if (root.consecutiveFetchErrors >= 3 && !root.fetchErrorNotificationShown) {
                    ...
                    const title = "Illogical-mango Update Check Failed"
