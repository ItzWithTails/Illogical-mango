...
- **مسار تثبيت وترقية حقيقي.** يتكفّل `./install` بالاعتماديات وإعداد النظام؛ و`ilmango update`
...
- **Kira**: تميمة اختيارية برسوم البكسل، تتجول عند حواف الشاشة وتتفاعل مع ما تفعله. مطفأة افتراضيًا، وحزمة الرسوم بحجم ~32 ميبي بايت تُنزَّل على حدة من `./install` › Extras
...
cd Illogical-mango
./install       # تفاعلي، يسأل قبل كل خطوة
./install -y    # تلقائي، بلا أسئلة
```
...
```bash
./install                 # قائمة TUI، اختر ما تريد
./install --disable mango    # لا تمسّ إعدادات mango إطلاقًا
sudo make install       # تثبيت على مستوى النظام بدل مجلد المنزل
./install --rollback        # التراجع عن آخر تحديث
```
