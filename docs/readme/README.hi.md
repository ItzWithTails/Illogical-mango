...
- **असली इंस्टॉल और अपग्रेड का रास्ता।** `./install` निर्भरताएँ और सिस्टम कॉन्फ़िग सँभालता है;
  `ilmango update` pull करता है, स्कीमा माइग्रेशन चलाता है, आपके बदलाव बचाए रखता है और रोलबैक भी
  कर सकता है।
...
गया) → यह, MangoWM पर पोर्ट किया हुआ। CLI, कॉन्फ़िग पथ और अंदरूनी हिस्से `ilmango` कहलाते हैं।
iNiR के ज़माने के इंस्टॉल को माइग्रेशन 037 आगे ले जाता है; वह पुराने पथों पर सिमलिंक छोड़ देता है,
इसलिए मौजूदा कीबाइंड और स्क्रिप्ट चलते रहते हैं।
...
`~/.config/mango/ilmango.conf` में रखता है और उसी की ओर इशारा करती एक `source-optional=` पंक्ति
जोड़ देता है, आपकी विंडो व्यवस्था को छुए बिना। ऑटोस्टार्ट उसी फ़ाइल में
`exec-once=ilmango run --daemon` पंक्ति है, कोई systemd यूनिट नहीं।
...
- **8 दृश्य शैलियाँ**: Material (ठोस), Cards, Aurora (काँच जैसा धुँधलापन), Illogical-mango (TUI अंदाज़), Angel (नियो-ब्रूटलिज़्म), Regalia (काला इंजीनियरिंग ढाँचा, गर्म हाथीदाँत रंग की स्याही, संयत शैम्पेन धातु), ZZZ (पोस्टर जैसी पट्टियाँ), Cookie Shapes (आकृतियों का सजीव रूपांतरण)
...
- **अपने आप अपडेट**: `ilmango update`, रोलबैक, माइग्रेशन और आपके बदलावों की हिफ़ाज़त के साथ
...
- **Kira**: चाहें तो चालू कीजिए — पिक्सेल आर्ट शुभंकर, जो स्क्रीन के किनारों पर घूमती है और आपकी हरकतों पर प्रतिक्रिया देती है। डिफ़ॉल्ट रूप से बंद; करीब 32 MiB का आर्ट पैक `./install` › Extras से अलग से उतरता है
...
cd Illogical-mango
./install       # संवादात्मक, हर क़दम से पहले पूछता है
./install -y    # अपने आप, कुछ नहीं पूछता
```

इंस्टॉलर निर्भरताएँ, सिस्टम कॉन्फ़िग और थीमिंग सँभालता है। वह शेल के कीबाइंड
`~/.config/mango/ilmango.conf` में लिखता है और आपके मौजूदा mango कॉन्फ़िग से जोड़ देता है, आपकी
विंडो व्यवस्था को छुए बिना। इसके बाद mango दोबारा चालू कीजिए या
`mmsg dispatch reload_config` चलाइए।

```bash
ilmango run                        # शेल चालू कीजिए
ilmango settings                   # सेटिंग्स GUI खोलिए
ilmango logs                       # लॉग देखिए
ilmango doctor                     # अपने आप जाँच और मरम्मत
ilmango update                     # pull + माइग्रेशन + दोबारा चालू
```

और रास्ते:

```bash
./install                 # TUI मेन्यू, जो चाहिए वह चुनिए
./install --disable mango    # mango कॉन्फ़िग को बिल्कुल न छुएँ
sudo make install       # घर की जगह पूरे सिस्टम में
./install --rollback        # पिछला अपडेट पलट दीजिए
```
...
```bash
ilmango logs                       # हाल के रनटाइम लॉग
ilmango restart                    # चालू रनटाइम दोबारा शुरू कीजिए
ilmango repair                     # doctor + दोबारा शुरू + छाने हुए लॉग की जाँच
ilmango doctor                     # आम दिक़्क़तों की अपने आप जाँच और मरम्मत
./install --rollback                # पिछला अपडेट पलट दीजिए
