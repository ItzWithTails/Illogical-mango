...
#
# Delegates to `ilmango restart` which correctly kills the wrapper process
...
launcher_script="$config_dir/scripts/ilmango"

if [[ ! -f "$launcher_script" ]]; then
    launcher_script="$script_dir/ilmango"
fi

if [[ ! -f "$launcher_script" ]]; then
    echo "restart-shell: cannot find ilmango launcher" >&2
    exit 1
fi
