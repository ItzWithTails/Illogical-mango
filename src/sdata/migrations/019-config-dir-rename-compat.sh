...
MIGRATION_TITLE="Config directory compatibility for ~/.config/ilmango"
MIGRATION_DESCRIPTION="Ensures config directory compatibility by linking legacy ~/.config/illogical-impulse to ~/.config/ilmango while preserving user data."
MIGRATION_TARGET_FILE="~/.config/ilmango"
MIGRATION_REQUIRED=true

migration_check() {
  local xdg_config_home="${XDG_CONFIG_HOME:-$HOME/.config}"
  local config_new="${xdg_config_home}/ilmango"
...
migration_preview() {
  ...
  echo "  - canonical: ${xdg_config_home}/ilmango"
  echo "  - legacy link: ${xdg_config_home}/illogical-impulse -> ${xdg_config_home}/ilmango"
}
