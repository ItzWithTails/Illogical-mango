...
migration_apply() {
  ...
  export ILMANGO_MIGRATION_NIRI_CONFIG="$config"
  export ILMANGO_MIGRATION_LAUNCHER_PATH="${XDG_BIN_HOME:-$HOME/.local/bin}/ilmango"
  python3 << 'MIGRATE'
import os
import shutil
import time
...
config_path = Path(os.environ["ILMANGO_MIGRATION_NIRI_CONFIG"]).expanduser()
launcher_path = os.environ["ILMANGO_MIGRATION_LAUNCHER_PATH"]
...
binds_text = normalize_text(buckets["70"]).replace(
    ...
    'spawn "bash" "-lc" "exec \\"$(ilmango path)/scripts/launch-terminal.sh\\""',
    f'spawn "{launcher_path}" "terminal"',
).replace(
    'spawn "bash" "-lc" "exec \\"$(ilmango path)/scripts/close-window.sh\\""',
...
// ─────────────────────────────────────────────────────────────────────────────
//
// This file is YOURS. Illogical-mango updates will never overwrite it.
