# System setup for Illogical-mango
...
function setup_super_daemon(){
  ...
  local daemon_src="${REPO_ROOT}/scripts/daemon/ilmango_super_overview_daemon.py"
  local service_src="${REPO_ROOT}/scripts/systemd/ilmango-super-overview.service"
  local daemon_dst="${HOME}/.local/bin/ilmango_super_overview_daemon.py"
  local service_dst="${XDG_CONFIG_HOME}/systemd/user/ilmango-super-overview.service"
  ...
  if [[ -n "${DBUS_SESSION_BUS_ADDRESS}" ]]; then
    v systemctl --user daemon-reload
    v systemctl --user enable ilmango-super-overview.service --now
  else
    log_warning "Not in graphical session. Enable later with:"
    echo "  systemctl --user enable ilmango-super-overview.service --now"
  fi
...
function setup_desktop_settings(){
  ...
  # Keep default icon theme aligned with Illogical-mango defaults/config and installer payload.
