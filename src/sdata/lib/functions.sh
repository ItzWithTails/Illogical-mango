# Core functions for Illogical-mango installer
...
cp_file(){
  ...
  # (e.g. when ~/.config/quickshell/ilmango points into the repo).
  if [[ -e "$dst" ]]; then
...
function ensure_launcher_path_in_shells(){
  ...
  local marker="# Illogical-mango launcher PATH"
  local end_marker="# end Illogical-mango launcher PATH"
  local sh_block="
${marker}
case \":\$PATH:\" in
  *:\"${launcher_dir}\":*) ;;
...
"
  ...
  cat > "${fish_conf_dir}/ilmango-path.fish" << EOF
