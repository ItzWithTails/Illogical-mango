# Conflict detection for Illogical-mango
...
check_conflicts() {
    ...
    # Quickshell fork or config that directly conflicts with Illogical-mango at runtime.
    ...
    # Packages that are critical conflicts — they break Illogical-mango at the package
    ...
    if [ ${#conflicts[@]} -gt 0 ]; then
        ...
        if [ ${#critical_conflicts[@]} -gt 0 ]; then
            echo -e "${STY_RED}${STY_BOLD}Critical:${STY_RST} ${critical_conflicts[*]} must be removed for Illogical-mango to work."
            ...
            if [[ "$package_manager" == "pacman" ]]; then
                ...
                if [[ "$ask" == "true" ]]; then
                    if tui_confirm "Remove conflicting packages? (${ordered_remove[*]})"; then
                        ...
                        echo -e "${STY_YELLOW}Keeping conflicting packages — Illogical-mango will NOT work correctly.${STY_RST}"
                    fi
                else
        ...
        echo -e "${STY_BOLD}Illogical-mango provides its own bar, notifications, wallpaper, and shell.${STY_RST}"
