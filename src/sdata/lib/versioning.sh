#!/bin/bash
# Versioning system for Illogical-mango
...
CONFIG_DIR_NEW="${XDG_CONFIG_HOME_RESOLVED}/ilmango"
CONFIG_DIR_PREV="${XDG_CONFIG_HOME_RESOLVED}/inir"
...
elif [[ -d "$CONFIG_DIR_LEGACY" && ! -L "$CONFIG_DIR_LEGACY" ]]; then
    CONFIG_DIR="$CONFIG_DIR_LEGACY"
elif [[ -d "$CONFIG_DIR_PREV" && ! -L "$CONFIG_DIR_PREV" ]]; then
    CONFIG_DIR="$CONFIG_DIR_PREV"
...
RUNTIME_DIR_USER="${XDG_CONFIG_HOME_RESOLVED}/quickshell/ilmango"
RUNTIME_DIR_SYSTEM_LOCAL="${ILMANGO_SYSTEM_RUNTIME_DIR_LOCAL:-/usr/local/share/quickshell/ilmango}"
RUNTIME_DIR_SYSTEM="${ILMANGO_SYSTEM_RUNTIME_DIR:-/usr/share/quickshell/ilmango}"
LEGACY_RUNTIME_DIR_USER="${XDG_CONFIG_HOME_RESOLVED}/quickshell/ii"
LEGACY_RUNTIME_DIR_SYSTEM_LOCAL="${ILMANGO_LEGACY_SYSTEM_RUNTIME_DIR_LOCAL:-/usr/local/share/quickshell/ii}"
LEGACY_RUNTIME_DIR_SYSTEM="${ILMANGO_LEGACY_SYSTEM_RUNTIME_DIR:-/usr/share/quickshell/ii}"
# iNiR-era payload directories, before migration 037 moved them to .../quickshell/ilmango
PREV_RUNTIME_DIR_USER="${XDG_CONFIG_HOME_RESOLVED}/quickshell/inir"
PREV_RUNTIME_DIR_SYSTEM_LOCAL="${ILMANGO_PREV_SYSTEM_RUNTIME_DIR_LOCAL:-/usr/local/share/quickshell/inir}"
PREV_RUNTIME_DIR_SYSTEM="${ILMANGO_PREV_SYSTEM_RUNTIME_DIR:-/usr/share/quickshell/inir}"
...
GITHUB_REPO="ItzWithTails/illogical-mango"
GITHUB_API="https://api.github.com/repos/${GITHUB_REPO}"

# Cache for remote version checks (avoid hammering GitHub)
VERSION_CACHE_FILE="${XDG_CACHE_HOME:-$HOME/.cache}/ilmango/version-cache.json"
VERSION_CACHE_TTL=3600  # 1 hour in seconds

get_runtime_shell_dir() {
    local override="${ILMANGO_RUNTIME_DIR:-}"
    ...
    for candidate in "$PREV_RUNTIME_DIR_USER" "$PREV_RUNTIME_DIR_SYSTEM_LOCAL" "$PREV_RUNTIME_DIR_SYSTEM" \
                     "$LEGACY_RUNTIME_DIR_USER" "$LEGACY_RUNTIME_DIR_SYSTEM_LOCAL" "$LEGACY_RUNTIME_DIR_SYSTEM"; do
        if [[ -n "$candidate" && -f "$candidate/shell.qml" ]]; then
            printf '%s' "$candidate"
            return
        fi
...
get_installed_version_file() {
    ...
    if [[ -n "${ILMANGO_RUNTIME_DIR:-}" && -f "$runtime_version_file" ]]; then
...
get_default_package_update_hint() {
    case "$(get_installed_package_manager)" in
        ...
        *) echo "use your package manager to update Illogical-mango" ;;
    esac
}
...
get_install_mode() {
    if [[ -n "${ILMANGO_INSTALL_MODE:-}" ]]; then
        echo "$ILMANGO_INSTALL_MODE"
        return
    fi

    local target="${XDG_CONFIG_HOME_RESOLVED}/quickshell/ilmango"
    local repo_real=""
    local target_real=""
...
get_update_strategy() {
    if [[ -n "${ILMANGO_UPDATE_STRATEGY:-}" ]]; then
        echo "$ILMANGO_UPDATE_STRATEGY"
        return
    fi
...
get_version_repo_path() {
    if [[ -n "${ILMANGO_REPO_PATH:-}" ]]; then
        printf '%s' "$ILMANGO_REPO_PATH"
        return
    fi
...
write_version_info_json() {
    ...
    local package_manager="${ILMANGO_PACKAGE_MANAGER:-}"
    local package_name="${ILMANGO_PACKAGE_NAME:-}"
    local package_update_hint="${ILMANGO_PACKAGE_UPDATE_HINT:-}"
...
show_version_status() {
    ...
    echo ""
    echo -e "${STY_CYAN}${STY_BOLD}Illogical-mango Version Status${STY_RST}"
    echo ""
...
show_changelog() {
    ...
    echo -e "${STY_CYAN}${STY_BOLD}Illogical-mango Changelog${STY_RST}"
    echo ""
