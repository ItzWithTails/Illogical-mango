# User modification detection for Illogical-mango
...
manifest_has_checksums() {
    ...
    # v2 manifests may have either the legacy ii header or the current ilmango header
    head -1 "$manifest_file" | grep -qE "(ii|ilmango)-manifest v2" && return 0
...
find_runtime_manifest_file() {
    local target_dir="$1"
    local manifest
    for manifest in "${target_dir}/.ilmango-manifest" "${target_dir}/.ii-manifest"; do
        if [[ -f "$manifest" ]]; then
            printf '%s\n' "$manifest"
            return 0
        fi
...
show_file_diff() {
    ...
    local original="${snapshot_dir}/ilmango/${file_path}"
    if [[ ! -f "$original" ]]; then
    ...
    while true; do
        ...
        case "$choice" in
            "Preserve & Continue")
                ...
                tui_dim "    cp $PRESERVED_MODS_DIR/<path> ~/.config/quickshell/ilmango/<path>"
                return 0
                ;;
            "View Changes")
                ...
                if [[ -n "$latest_snapshot" ]] && [[ -d "${SNAPSHOTS_DIR}/${latest_snapshot}/ilmango" ]]; then
                    echo ""
                    local shown=0
