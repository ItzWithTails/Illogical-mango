...
Singleton {
    ...
    readonly property string previewFile: "/tmp/ilmango-gowall-preview"
    ...
    readonly property string _shimDir: "/tmp/ilmango-gowall-shim"
    ...
    function convertCurrentTheme(sourcePath: string, format: string): void {
        ...
        if (palette.length === 0) { error = "Current Illogical-mango theme palette is unavailable"; return }
        const json = JSON.stringify({ name: "ilmango-current", colors: palette }, null, 2)
        const fmt = _ext(format)
        _runOperation(["/usr/bin/gowall", "convert", src, "--theme", _customThemePath, "--output", _previewPathFor(fmt), "--format", fmt], fmt, json, true, _basenameWithoutExt(src))
    }
